...
public class LodowaLanca extends Rune {

	public LodowaLanca(DynamicRune dr, Player p) {
		super(dr, p);
	}
	...
	public void castSpell() {
		...
		new BukkitRunnable() {
			...
			public void run() {
				...
				double z = vec.getZ()*t;
				loc.add(x,y,z);
				ParticleEffect.SNOWBALL.display(loc,30,0,0,0,0,10);
...
//				loc.getWorld().playSound(loc, Sound.FIRE_IGNITE, 1, 1);
				...
				z = vec.getZ()*t;
				loc.add(x,y,z);
				ParticleEffect.SNOWBALL.display(loc,60,0,0,0,0,10);
				ParticleEffect.SNOWBALL.display(loc,30,0.5F,0.5F,0.5F,0.05F,10);
//				loc.getWorld().playSound(loc, Sound.FIRE_IGNITE, 1, 1);
				for(Entity e:loc.getChunk().getEntities()) {
					if(e.getLocation().distance(loc) < 1.0 || e.getLocation().add(0, 1, 0).distance(loc) < 1.0) {
