...
public class WyostrzoneZmysly extends Rune{

	public WyostrzoneZmysly(DynamicRune dr, Player p) {
		super(dr, p);
	}
			...
			public void run() {
				...
				for(double y = 0.1;y<=2.2;y+=0.1) {
					for(double t = 0;t<=2*Math.PI;t+=(Math.PI/16)) {
						...
						double z = Math.cos(t);
						loc.add(x,y,z);
						ParticleEffect.SMOKE_LARGE.display(loc, 10, 0, 0, 0, 0, 1);
						loc.subtract(x, y, z);
					}
				}
			}
