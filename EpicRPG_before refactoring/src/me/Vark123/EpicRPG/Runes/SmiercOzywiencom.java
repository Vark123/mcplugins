...
public class SmiercOzywiencom extends Rune {

	public SmiercOzywiencom(DynamicRune dr, Player p) {
		super(dr, p);
	}
	...
	public void castSpell() {
		...
		new BukkitRunnable() {
			...
			public void run() {
				...
				ParticleEffect.SMOKE_LARGE.display(loc,30,0.5F,0.5F,0.5F,0.2F,10);
//				loc.getWorld().playSound(loc, Sound.FIRE_IGNITE, 1, 1);
				for(Entity e:loc.getChunk().getEntities()) {
					if(e.getLocation().distance(loc) < 1.5 || e.getLocation().add(0, 1, 0).distance(loc) < 1.5) {
						if(!e.equals(p) && e instanceof LivingEntity) {
							ParticleEffect.SMOKE_LARGE.display(loc,30,1F,1F,1F,0.4F,50);
							loc.getWorld().playSound(loc, Sound.BLAZE_BREATH, 1, 1);
